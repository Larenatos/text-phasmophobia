...
You can download an executable from [releases](https://github.com/Larenatos/text-phasmophobia/releases) and there you can find out more information about the features to the game.

You can also run this game with intellij IDEA by opening the repository in there.

To run the game in IDEA, go to `src` / `textPhasmophobia` / `ui` / `TextUI` from the repository root.
Specifically you need to run the main class `TextUI` in file `TextUI.scala`
